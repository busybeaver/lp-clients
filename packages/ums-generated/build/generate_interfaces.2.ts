...
type CodeGeneratorFn = (schema: any, sourceFile: SourceFile) => void;
type SchemaType = {
  url: string,
  fn?: CodeGeneratorFn,
};
...
const generateSchema = async (schemaType: SchemaType): Promise<ParsedSchemaType> => {
...
const commonData = (schema: any, sourceFile: SourceFile, baseName: string, interfaceName: string): string[] => {
...
const customNotificationData = (baseName: string, schema: any, sourceFile: SourceFile): void => {
  ...
  const events: string[] = commonData(schema, sourceFile, pascalName, notificationTypeName);
...
const customConsumerNotificationData: CodeGeneratorFn = (schema, sourceFile) => customNotificationData("consumer", schema, sourceFile);
const customAgentNotificationData: CodeGeneratorFn = (schema, sourceFile) => customNotificationData("agent", schema, sourceFile);

const customConsumerRequestData: CodeGeneratorFn = (schema, sourceFile) => {
  const baseName = "ConsumerRequests";
  commonData(schema, sourceFile, baseName, iSendTypeName);
};

const customConsumerResponseData: CodeGeneratorFn = (schema, sourceFile) => {
  const baseName = "ConsumerResponses";
...
const generateGenericTypings: CodeGeneratorFn = (schema, sourceFile) => {
...
const customAgentResponseData: CodeGeneratorFn = (schema, sourceFile) => {
  const baseName = "AgentResponses";
  commonData(schema, sourceFile, baseName, iResponseTypeName);
...
(async () => {
  try {
    ...
    generateGenericTypings(null, sourceFile);
    schemaTypes.filter((schemaType) => typeof schemaType.fn === "function")
      .forEach((schemaType) => Reflect.apply(schemaType.fn!, null, [schemaType.schema, sourceFile]));
