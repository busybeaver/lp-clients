...
const generateTypings = async ({url, fn}: schemaType): Promise<string> => {
  ...
  const file = join(process.cwd(), "lib", "generated", fileName + ".ts");
  ...
  // await outputFile(join(process.cwd(), "lib", "generated", filename), header + typings + eventTypesEnum + customContent);
...
const customConsumerResponseData = (schema: any, sourceFile: SourceFile, fileName: string): void => {
  ...
  classConfig.methods = Object.entries(mapping).map(([req, res], a, b): MethodDeclarationStructure => {
    return {
...
const generateGenericTypings = async (): Promise<string> => {
  const ast = new Ast();
  const file = join(process.cwd(), "lib", "generated", `${commonUmsFileName}.ts`);
  await remove(file); // remove old/previous versions
...
(async () => {
  try {
    ...
    const ast = new Ast();
    const file = join(process.cwd(), "lib", "generated", "index.ts");
    await remove(file); // remove old/previous versions
    const sourceFile = ast.createSourceFile(file);
